...
def main():
    ...
    print 'Tenho que adicionar attrs no Gene_content...'
    meu_gtf = GTF_manager(gtf_file='/home/lucas/PycharmProjects/dinar_correlacoes_20016/dados/results/genes-2.gtf')

    #print meu_gtf.genes_hash['XLOC_013994'].get_bed6()
    #gtf_object.print_attrs_fields()
    # meu_gtf.gene_list_to_bed6(save_in_file=True, file_name='teste_genes.bed')
    # meu_gtf.transcripts_list_to_bed6(save_in_file=True, file_name='teste_transcripts.bed')
    import random

    pc = open('/home/lucas/PycharmProjects/dinar_correlacoes_20016/dados/results/test03.txt').read().replace(',', '.').split('\n')[1:]

    lncrnas = open('/home/lucas/PycharmProjects/dinar_correlacoes_20016/dados/results/test04.txt').read().replace(',', '.').split('\n')[1:]

    pc_hash = {pc_gene.split('\t')[0]: pc_gene.split('\t')[1:] for pc_gene in pc}
    lncrnas_hash = {ln_gene.split('\t')[0]: ln_gene.split('\t')[1:] for ln_gene in lncrnas}

    pc_list = [pc_gene.split('\t')[0] for pc_gene in pc]
    lncrnas_list = [ln_gene.split('\t')[0] for ln_gene in lncrnas]

    lncrnas_gtf = copy.deepcopy(meu_gtf)
    pc_gtf = copy.deepcopy(meu_gtf)
    lncrnas_gtf.gene_list_select(lncrnas_list).gene_list_to_bed6(save_in_file=True, file_name='lncrnas.bed')
    bed6_lncrnas = lncrnas_gtf.gene_list_select(lncrnas_list).gene_list_to_bed6()

    bed6_pc = pc_gtf.gene_list_select(pc_list).gene_list_to_bed6()
    ...
    for vizinhos in bed6_pc.closest(bed6_pc, io =True, d=True):
        #print 'pc', 'distance:', vizinhos[-1]

        chave_pc = '_'.join(sorted([vizinhos[3], vizinhos[9]]))
        if chave_pc not in pc_slot:
            x = map(float, pc_hash[vizinhos[3]])
            y = map(float, pc_hash[vizinhos[9]])
            print '_________', chave_pc, '-----------'
            print vizinhos
            print x
            print y
    ...
    for vizinhos in bed6_lncrnas.closest(bed6_pc, io=True, d=True):
    ...
    print len(pair_lncrnas), len(pair_protein)


    sns.distplot(np.array(pair_protein), hist=False, color="r",  kde_kws={"shade": False, 'lw': 3, 'alpha': 0.7}, label='protein_protein')
    sns.distplot(np.array(pair_lncrnas), hist=False, color="b",  kde_kws={"shade": False, 'lw': 3, 'alpha': 0.7}, label='lincRNA_protein')
    ...
    #     print gene_id.gene_id, gene_id.locus_coords(), gene_id.locus_length(), gene_id.get_bed6()
